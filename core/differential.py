...
# [FIX C.3] Relaxed composite_z threshold used when Phase 3 (waf_detector.py)
# identified a WAF/bot-mitigation vendor in front of the candidate. A WAF
# adds its own response-shape noise (challenge pages, rate-limit delays,
# generic block-page timing) on top of whatever the origin app does, which
# widens the natural variance around the "normal" baseline. Using the same
# 2.0 threshold as an unprotected target measurably raises false positives
# on WAF-fronted candidates; correlating the two signals (previously they
# were computed independently and never cross-referenced) lets the same
# composite_z math stay meaningful in both cases instead of picking one
# threshold that's wrong for whichever case wasn't tuned for.
SUSPICIOUS_THRESHOLD_WAF = 2.5
...
def is_suspicious(
    result: ProbeResult, candidate: Candidate | None = None, *,
    threshold: float = SUSPICIOUS_THRESHOLD,
) -> bool:
    """
    Returns True if a result warrants Phase 5/6 follow-up.

    [FIX C.3] `threshold`: caller-supplied composite_z bar, defaulting to
      SUSPICIOUS_THRESHOLD. core/agent.py passes SUSPICIOUS_THRESHOLD_WAF
      instead when Phase 3 (waf_detector.py) identified a WAF vendor on
      this candidate — see that constant's docstring above for why. The
      two Phase 3/Phase 4 signals were previously computed independently
    ...
    if result.composite_z >= threshold:
        ...
        return result.composite_z >= threshold * 1.5  # higher bar
