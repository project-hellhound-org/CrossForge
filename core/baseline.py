...
async def establish_baseline(client, candidate: Candidate) -> BaselineProfile:
    ...
    is already called once per `candidate` (core/agent.py Phase 1), using
    THIS candidate's own `original_value` — it is per-parameter by
    construction, not global. The reflection concern itself is handled by
    two other mechanisms instead of a baseline-time reflection flag:
      1. content_length_stddev's floor (see finalize(), 2% of mean, min
         8 bytes) absorbs the response-size noise a reflecting parameter
         produces, since the SAME value is sent every baseline sample.
      2. A dedicated reflection check runs on live probe results (not
         baseline samples) in agent.py's per-candidate loop — comparing
         the actual injected payload against the response body — which is
         a more accurate signal than testing reflection with a synthetic
         baseline value, since it observes the real payload's fate.
    No code change made for C.2; documenting this so a future audit doesn't
