...
_PARAM_RULES: list[tuple[VulnType, float, frozenset]] = [
    ...
    # --- File upload SSRF (SVG / DOCX / XML content-based SSRF)
    (VulnType.FILE_UPLOAD_SSRF, 0.91, frozenset({
        "file", "attachment", "document", "docx", "svg", "xml",
        "resume", "cv", "certificate", "template", "spreadsheet",
    })),
...
_PATH_RULES: list[tuple[VulnType, float, frozenset]] = [
    ...
    (VulnType.FILE_UPLOAD_SSRF, 0.90, frozenset({
        "upload", "uploads", "attach", "attachment", "attachments",
        "document", "documents", "file", "files", "resume",
        "ingest", "asset", "assets",
    })),
...
_CATEGORY_LABELS: dict[VulnType, str] = {
    ...
    VulnType.FILE_UPLOAD_SSRF:   "File Upload SSRF (SVG/DOCX/XML Content)",
