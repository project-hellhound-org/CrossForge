...
def classify_response(
    baseline: QualityBaseline, status: int, body: str, *, is_seed: bool = False,
) -> QualityVerdict:
    """
    ...
    [FIX B.1] `is_seed`: True for the base URL and any URL sourced from
    robots.txt/sitemap.xml/security.txt/Wayback — i.e. an operator- or
    crawler-confirmed real target, not a guess. On SPA-routed targets
    (client-side hash routing), the server returns the identical
    `index.html` shell for every unmatched path, including the canary probe
    used to establish `baseline.canary_hash`. Without this exemption, the
    FIRST real page fetched hashes identically to the canary and gets
    classified SOFT_404 — discarded before link/form/JS parsing ever runs,
    on every SPA target, silently. Seed URLs are known-real by construction
    (they came from the base URL or an authoritative discovery source), so
    they always bypass the canary-hash check; they can still be caught by
    the independent bot-block marker check and the duplicate-shell check
    below (those signatures don't depend on the canary and stay valid even
    for seeds).
    """
    ...
    if (not is_seed and baseline.established
            and status == baseline.canary_status and body_hash == baseline.canary_hash):
