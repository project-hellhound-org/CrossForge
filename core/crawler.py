...
class NativeCrawler:
    ...
    def __init__(self, cfg: CrawlConfig, rate_limiter: RateLimiter | None = None,
        ...
        self._js_inline_blocks_analyzed = 0   # [FIX B.3] see _analyze_js_text call site below
    ...
    async def crawl(self, base_url: str) -> dict:
        ...
        # [FIX B.1/B.2] frontier entries are now (url, depth, is_seed).
        # is_seed marks URLs that are known-real by construction (the base
        # URL the operator gave us, or a URL sourced from an authoritative
        # discovery channel) rather than organically found by following
        # links from parsed content. This flag threads through to two
        # independent fixes:
        #   B.1 — is_seed URLs skip the canary-hash SOFT_404 check in
        #         recon_quality.classify_response() (SPA shell collision).
        #   B.2 — is_seed URLs are excluded from AdaptiveBudget.record_level()
        #         scoring so a batch of seed-sourced leaf pages can't tank
        #         the yield ratio and stall the crawl before organic BFS
        #         ever gets a fair measurement.
        frontier: list[tuple[str, int, bool]] = [(str(base), 0, False)]  # (url, depth, is_seed)
        ...
        while frontier and depth <= self.cfg.max_depth and budget.has_room():
            ...
            results = await asyncio.gather(*[
                self._fetch_and_parse(url, sem, is_seed=is_seed)
                for url, _d, is_seed in this_level
            ], return_exceptions=True)
            budget.consume(len(this_level))

            # [FIX B.2] Track organic (non-seed) fetch/yield counts
            # separately from the full batch. Every seed source (robots,
            # sitemap, security.txt, Wayback) is injected at depth 0
            # alongside the base URL, and seed URLs are frequently
            # leaf/asset pages that yield zero fresh links by nature —
            # scoring the whole mixed batch let a seed-heavy level 0
            # tank the yield ratio and stall the crawl before a single
            # organically-discovered link was ever explored.
            organic_fetched_this_level = 0
            new_links: list[str] = []
            new_links_organic = 0
            for (url, d, is_seed), res in zip(this_level, results):
                if not is_seed:
                    organic_fetched_this_level += 1
            ...
            # Seeds still consume budget (ensuring diversity of starting
            # points) but never drive the stall decision — only organic
            # fetch/yield counts do. If this level was 100% seed URLs,
            # skip scoring entirely rather than dividing by zero / scoring
            # on a sample size of zero organic pages.
            if organic_fetched_this_level > 0:
                budget.record_level(organic_fetched_this_level, new_links_organic)
            depth += 1
            frontier.extend((u, depth, False) for u in new_links)  # discovered links are always organic
        ...
        meta = {
            ...
            "crawl_stats": {
                ...
                "js_inline_blocks_analyzed": self._js_inline_blocks_analyzed,  # [FIX B.3]
        ...
        try:
            async with async_playwright() as pw:
                ...
                for seed in seed_pages[: self.cfg.headless_max_seed_pages]:
                    ...
                    # [FIX B.4] Form field extraction. The link/action capture
                    # above sees `[action]` and records the form's target URL,
                    # but never walks the form's own `<input>/<select>/
                    # <textarea>` descendants — so a rendered form became a
                    # param-less candidate and precision (which field name
                    # actually carries a URL, e.g. `avatar_url`) was lost.
                    # Read-only DOM inspection only — no submission, no
                    # interaction, matching the "no form-fill, no clicking"
                    # principle stated in this method's docstring.
                    try:
                        form_data = await page.eval_on_selector_all(
                            "form",
                            "forms => forms.map(f => ({action: f.action, method: f.method, "
                            "fields: Array.from(f.querySelectorAll('[name]')).map(el => el.name)"
                            ".filter(Boolean)}))",
                        )
                        for f in form_data:
                            action = f.get("action") or seed
                            abs_action = (
                                action if action.startswith(("http://", "https://"))
                                else _resolve(seed, action)
                            )
                            fields = f.get("fields") or []
                            if abs_action and _in_scope(abs_action, allowed_hosts) and fields:
                                method = (f.get("method") or "GET").upper()
                                bucket = "query" if method == "GET" else "form"
                                self._record_endpoint_fields(abs_action, method, bucket, fields)
                    except Exception:
                        pass
                    ...
                    for route in list(hash_routes_discovered)[:_MAX_HASH_ROUTES]:
                        try:
                            ...
                            try:
                                ...
                                pass
                            # [FIX B.4] Same form-field extraction as the
                            # initial render — a settings/admin panel behind
                            # a client-side route (e.g. #/settings/webhooks)
                            # is exactly where SSRF-relevant form fields tend
                            # to live and would otherwise never be captured.
                            try:
                                route_forms = await page.eval_on_selector_all(
                                    "form",
                                    "forms => forms.map(f => ({action: f.action, method: f.method, "
                                    "fields: Array.from(f.querySelectorAll('[name]')).map(el => el.name)"
                                    ".filter(Boolean)}))",
    ...
    async def _fetch_and_parse(
        self, url: str, sem: asyncio.Semaphore, *, is_seed: bool = False,
        ...
        if self.cfg.quality_gate_enabled and self._quality_baseline is not None:
            verdict = recon_quality.classify_response(
                self._quality_baseline, resp.status_code, body, is_seed=is_seed,
            )
        ...
        if self.cfg.js_static_analysis:
            ...
            for inline in parser.inline_js:
                ...
                self._js_inline_blocks_analyzed += 1  # [FIX B.3]
...
async def run_crawl(
    ...
    ok(
        ...
        f"{color(str(stats['js_files_analyzed']), C.BWHITE)} JS files + "
        f"{color(str(stats.get('js_inline_blocks_analyzed', 0)), C.BWHITE)} inline JS blocks analyzed, "  # [FIX B.3]
    ...
    # [FIX B.5] Pages fetched but nothing parsed from any of them reads as a
    # clean "recon complete, zero forms/JS" result — but it can also mean
    # the quality gate over-filtered every single page pre-parse (Issue B.1
    # class of bug). An operator can't tell those two situations apart from
    # the counters alone, so make the ambiguous case loud and specific.
    if (stats['pages_fetched'] > 0 and stats['forms_found'] == 0
            and stats['js_files_analyzed'] == 0
            and stats.get('js_inline_blocks_analyzed', 0) == 0):
        qg = stats.get('quality_gate') or {}
        qg_total = sum(qg.values()) if qg else 0
        warn(
            f"{stats['pages_fetched']} page(s) fetched but 0 forms/JS parsed from any of them — "
            f"quality gate filtered {qg_total} page(s) "
            f"(soft_404={qg.get('soft_404_filtered', 0)}, "
            f"duplicate_shell={qg.get('duplicate_shell_filtered', 0)}, "
            f"bot_blocked={qg.get('bot_blocked_filtered', 0)}) — "
            f"verify this isn't over-filtering before trusting downstream results."
        )
