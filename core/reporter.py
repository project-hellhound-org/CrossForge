...
def write_sarif_report(report: ScanReport, output_path: "str | Path") -> Path:
    ...
    sarif = {
        ...
        "runs": [{
            ...
            "results": results,
            # skipped_candidates surfaces here so CI pipelines and external
            # tooling consuming SARIF get the same incomplete-coverage signal
            # as the JSON report. write_sarif_report builds its own dict and
            # does not call to_dict(), so this field must be added explicitly.
            "properties": {
