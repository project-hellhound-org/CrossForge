...
async def _run_review(report_path: Path, config_path: Path) -> None:
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Help
    if args.help or (not args.input and not args.target and not args.review):
        print_banner()
        print_help()
        return

    # Review mode — no scan, no --input/target required. Short-circuits
    # here, same as --version/--help above, before the input/target
    # requirement check below (which doesn't apply to review mode).
    if args.review:
        ...
        if not config_path.exists():
            ...
            sys.exit(1)
        report_path = Path(args.review)
        try:
            asyncio.run(_run_review(report_path, config_path))
        except KeyboardInterrupt:
            tprint(f"\n  {color('Review session interrupted.', C.BYELLOW)}")
